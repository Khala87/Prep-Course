...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  for(var i = 0; i < array.length; i++){ //  return array.includes(elemento)
    if(array[i] === elemento){
      return true;
    }
  }
...
function agregarNumeros(numeros) {
  ...
  // Tu código:
  var acumulador = 0; //let suma = numeros.reduce((actual, nuevo) => + nuevo, 0);
  for(var i = 0; i < numeros.length; i++){ //return suma;
...
function numeroMasGrande(numeros) {
  ...
  for(var i = 0; i < numeros.length; i++){
    if(numeros[i] > mayor){
     ...
     return mayor; //return Math.max(...numeros)
}
...
function breakStatement(numero) {
  ...
  //for( var i = 1; i <= 10; i++){
    ...
    //if(acumulador === auxiliar){ break;}
    //auxiliar = auxiliar + 1;
    
    //miArray.push(acumulador)
  //}
  
  //if(acumulador = auxiliar) return "Se interrumpió la ejecución";
   //return miArray;
   var miArray = [];
   var acc = numero;
   var aux = 1;
